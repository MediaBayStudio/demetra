...
// Вместо введения этого ^ в терминале необходимо:
// - перейти в папку `Пользователи`, открыть папку с именем пользователя MacOS в редакторе кода
// - открыть файл `.bash_profile` и добавить строку `export PATH=$PATH:/Applications/MAMP/Library/bin/`
...
task('default', function(done) {
  ...
  watch(path.join(config.src.path, '**', '*.{json,lock}'), moveJSON);
