...
add_action( 'wp_enqueue_scripts', function() {
  ...
  if ( $GLOBALS['current_template'] === 'single-excursion' || $GLOBALS['current_template'] === 'taxonomy-tours_type') {
  ...
  // Подключаем скрипты циклом
	...
	$scripts = ['fancybox.min', 'lazy.min', 'tail.select.min', 'Popup.min', 'slick.min', 'script', $GLOBALS['page_script_name']];
...
// Убираем id и type в тегах script, добавляем нужным атрибут defer
  add_filter( 'script_loader_tag',   function( $html, $handle ) {
    ...
    if ($handle === 'fancybox.min' && $current_template !== 'single-excursion' && $GLOBALS['current_template'] !== 'taxonomy-tours_type') {
      return '';
    }

    if ( $handle === 'yandex-map-api' ) {
      $html = str_replace( "ru-RU", 'ru-RU&apikey=82596a7c-b060-47f9-9fb6-829f012a9f04', $html );
    }
